...
## Objective
Develop a production-ready MCP server using TypeScript to provide [SERVICE_NAME] functionality. The server must be scalable, maintainable, and follow best practices for code quality, error handling, and documentation, adhering to the MCP (Model-Controller-Presenter) pattern.

## Core Requirements

### 1. Server Setup
- Use @modelcontextprotocol/sdk for MCP server implementation.
- Implement in TypeScript with strict typing (strict: true in tsconfig.json).
...
- Testing: Include at least one unit test using Jest for the main tool.
...
- API documentation (OpenAPI spec or JSDoc).
- Example usage code.
- README.md with setup instructions and environment variable details.
- Unit tests for the main tool.
- Bolt XML output format for file generation.

## Example Implementation
Below is a minimal MCP server with a "greet" tool from the official MCP TypeScript SDK documentation, formatted as Bolt XML for file generation:
...
startServer().catch((error) => {
  ...
  <boltAction type="file" filePath="src/__tests__/greet.test.ts">
import { z } from 'zod';
import { GreetInputSchema } from '../index';

describe('Greet Tool', () => {
  test('validates input schema', () => {
    const validInput = { name: 'Alice' };
    expect(() => GreetInputSchema.parse(validInput)).not.toThrow();

    const invalidInput = { name: '' };
    expect(() => GreetInputSchema.parse(invalidInput)).toThrow('Name is required');
  });
});
  </boltAction>
...
- Provide complete, executable code with error handling, logging, and documentation in Bolt XML format.
